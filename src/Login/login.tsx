import React from "react";
import { useState } from "react";
import { useEffect } from "react";
import { Outlet, Link, useRoutes, useNavigate } from "react-router-dom";
import styled from "styled-components";

const Styledbox = styled.div`
  /* 박스 스타일*/
  width: 70%;
  height: 60%;
  border-radius: 28px;
  background-color: rgb(255, 255, 255);
  padding: 50px;

  position: absolute;
  top: 50%;
  left: 50%;
  transform: translate(-50%, -50%);
`;

const Styledconfirm = styled.div`
  width: 100%;
  height: 10%;
  display: flex;
  justify-content: right;
`;

/*비밀번호 찾기*/
const Styledbutton1 = styled.button`
  width: 119px;
  height: 48px;
  color: rgb(11, 87, 208);
  background-color: rgb(255, 255, 255);
  border: none;
`;

/*확인 키*/
const Styledbutton2 = styled.button`
  width: 76px;
  height: 48px;
  background-color: rgb(11, 87, 208);
  color: rgb(255, 255, 255);
  border: none;
  border-radius: 50px;
`;

const Styledback = styled.body`
  /* 배경*/
  background-color: rgb(240, 244, 249);
  display: flex;
  justify-content: center;
`;

const Styledcover = styled.div`
  display: flex;
  flex-direction: row;
  justify-content: space-between;
`;

const Styledname = styled.span`
  width: 108px;
  height: 45.5px;
  font-size: 46px;
`;

const Styledemail = styled.div`
  display: flex;
  flex-direction: row;
  width: 221px;
  height: 32px;
  gap: 10px;
  border: 1px solid black;
  border-radius: 50px;
`;

const StyledProfile = styled.button`
  width: 24px;
  height: 24px;
  border-radius: 50px;
  background-image: url("/img/myprofile.png");
  background-size: cover;
  border: none;
`;

const StyledInput = styled.input`
  border-radius: 3px;
  width: 100%;
  height: 52px;
  &::placeholder {
    position: relative;
    left: 5%;
  }

  border: 1px solid black;
`;

const StyledRight = styled.div`
  height: 364px;
  width: 300px;
  gap: 5px;
`;
const Styledcheck = styled.input`
  border: black;
`;
const Styledbuttonpass = styled.button`
  border: none;
  background-color: rgb(255, 255, 255);
  font-size: 14px;
`;

export default function SelfProve() {
  const [pass, setPass] = useState<string>("");
  const [state, setState] = useState<string>("waiting");
  const [check, setCheck] = useState<boolean>(false);
  const [toggle, setToggle] = useState<string>("password");
  const navigate = useNavigate();
  function checking(e: React.ChangeEvent<HTMLInputElement>) {
    setCheck(e.target.checked);
    if (!check) setToggle("input");
    else setToggle("password");
  }
  function passChange(e: React.ChangeEvent<HTMLInputElement>) {
    e.preventDefault();
    setPass(e.target.value);
  }
  useEffect(() => {
    console.log(state);
  }, [state]);

  function delay(f: void) {
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve(f);
      }, 2000);
    });
  }
  async function passConfirm() {
    await delay(setState("changing.."));
    if (pass === "010513") {
      setState("checking!");
      navigate("/main");
    } else {
      alert("error");
    }
  }
  return (
    <Styledback>
      <Styledbox>
        <svg
          xmlns="http://www.w3.org/2000/svg"
          x="0px"
          y="0px"
          width="48"
          height="48"
          viewBox="0 0 48 48"
        >
          <path
            fill="#FFC107"
            d="M43.611,20.083H42V20H24v8h11.303c-1.649,4.657-6.08,8-11.303,8c-6.627,0-12-5.373-12-12c0-6.627,5.373-12,12-12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C12.955,4,4,12.955,4,24c0,11.045,8.955,20,20,20c11.045,0,20-8.955,20-20C44,22.659,43.862,21.35,43.611,20.083z"
          ></path>
          <path
            fill="#FF3D00"
            d="M6.306,14.691l6.571,4.819C14.655,15.108,18.961,12,24,12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C16.318,4,9.656,8.337,6.306,14.691z"
          ></path>
          <path
            fill="#4CAF50"
            d="M24,44c5.166,0,9.86-1.977,13.409-5.192l-6.19-5.238C29.211,35.091,26.715,36,24,36c-5.202,0-9.619-3.317-11.283-7.946l-6.522,5.025C9.505,39.556,16.227,44,24,44z"
          ></path>
          <path
            fill="#1976D2"
            d="M43.611,20.083H42V20H24v8h11.303c-0.792,2.237-2.231,4.166-4.087,5.571c0.001-0.001,0.002-0.001,0.003-0.002l6.19,5.238C36.971,39.205,44,34,44,24C44,22.659,43.862,21.35,43.611,20.083z"
          ></path>
        </svg>
        <p></p>
        <Styledname>신혁수</Styledname>

        <Styledcover>
          <Styledemail>
            <StyledProfile></StyledProfile>sins051301@gamil.com
          </Styledemail>
          <StyledRight>
            <StyledInput
              type={toggle}
              placeholder="비밀번호 입력"
              onChange={passChange}
            ></StyledInput>
            <Styledcheck
              type="checkbox"
              checked={check}
              onChange={checking}
            ></Styledcheck>
            <Styledbuttonpass>비밀번호 표시</Styledbuttonpass>
          </StyledRight>
        </Styledcover>
        <Styledconfirm>
          <Styledbutton1>비밀번호 찾기</Styledbutton1>
          <Styledbutton2 onClick={passConfirm}> 다음</Styledbutton2>
        </Styledconfirm>
      </Styledbox>
    </Styledback>
  );
}
